...
public class MaterialesView extends VBox {
    ...
    private HBox buildToolbarStock() {
        ...
        HBox bar = new HBox(8, chkSoloAlerta, sp, btnEntrada, btnSalida, btnNuevo, btnEditar, btnBorrar, btnExportar);
        ...
        Thread.ofVirtual().start(() -> {
            try {
                switch (fmt[0]) {
                    ...
                    case "pdf"    -> ExportService.exportarMaterialesPDF(destino, dao.findAll());
                    case "word"   -> ExportService.exportarMaterialesWord(destino, dao.findAll());
                }
                Platform.runLater(() -> {
