...
$INPUT_DIR  = "$PROJECT\target\jpackage-input-v3"
...
& $ISCC "installer_v3.iss"
if ($LASTEXITCODE -ne 0) { Write-Error "ERROR: Inno Setup fallo"; exit 1 }

# --- Resultado ---------------------------------------------------------------
$exeV3 = "$PROJECT\output\GraficasMulberry-Instalador-v$VERSION.exe"
$exeV2 = "$PROJECT\output\GraficasMulberry-Instalador-v2.0.0.exe"
...
if (Test-Path $exeV2) {
    ...
    Write-Host ("  v2.0.0 (anterior): GraficasMulberry-Instalador-v2.0.0.exe  (" + $s2 + " MB)") -ForegroundColor Gray
}
if (Test-Path $exeV3) {
    $s3 = [math]::Round((Get-Item $exeV3).Length / 1MB, 1)
    Write-Host ("  v3.0.0 (nuevo):    GraficasMulberry-Instalador-v" + $VERSION + ".exe  (" + $s3 + " MB)") -ForegroundColor White
}
