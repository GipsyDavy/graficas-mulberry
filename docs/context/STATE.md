...
**Última actualización:** 2026-06-22 (Sprint B2-14 — PresupuestoDAO inyección Connection — 151/151 — Refactor B2 en 14/17 DAOs, cola de riesgo moderado CERRADA)
...
**HEAD:** commit Sprint B2-14 (más el commit de este STATE.md). Tests: 151/151. App funcional. Migración i18n de vistas: completa. Refactor B2 (Connection inyectada en DAOs): 14/17 DAOs migrados (`TarifaTramoDAO`, `NotaCalendarioDAO`, `ColumnConfigDAO`, `DynamicColumnValueDAO`, `ConsumoMaterialDAO`, `TarifaDAO`, `NominaDAO`, `PedidoDAO`, `MaterialDAO`, `PagoMaterialDAO`, `PagoPedidoDAO`, `EmpleadoDAO`, `ClienteDAO`, `PresupuestoDAO`). Cola de riesgo moderado CERRADA. Siguen los orquestadores `FacturaDAO`/`AlbaranDAO` (ambos con transacción manual propia, ambos ya referencian `PresupuestoDAO` migrado) y finalmente `UserDAO` (auth-sensible, `/security-review` extra). Patrón y orden de migración confirmados — ver Sprint B2-1→B2-14 arriba. Técnica de inicializador de campo + excepción comprobada (incl. constructores encadenados vía `this(...)` y try/catch autocontenido cuando el DAO hermano de la vista no está migrado) documentada y reutilizada en B2-2→B2-14. Los orquestadores siguen exigiendo Multi-IA real (Gemini+Codex) — no asumir mecánico sin consulta.
