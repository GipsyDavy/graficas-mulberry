...
**Última actualización:** 2026-06-22 (Sprint B2-15 — FacturaDAO inyección Connection — 151/151 — Refactor B2 en 15/17 DAOs, orquestador FacturaDAO CERRADO)
...
**HEAD:** commit Sprint B2-15 (más el commit de este STATE.md). Tests: 151/151. App funcional. Migración i18n de vistas: completa. Refactor B2 (Connection inyectada en DAOs): 15/17 DAOs migrados (`TarifaTramoDAO`, `NotaCalendarioDAO`, `ColumnConfigDAO`, `DynamicColumnValueDAO`, `ConsumoMaterialDAO`, `TarifaDAO`, `NominaDAO`, `PedidoDAO`, `MaterialDAO`, `PagoMaterialDAO`, `PagoPedidoDAO`, `EmpleadoDAO`, `ClienteDAO`, `PresupuestoDAO`, `FacturaDAO`). Cola de riesgo moderado CERRADA. Orquestador `FacturaDAO` CERRADO. Queda `AlbaranDAO` (último orquestador, transacción manual propia, ya referencia `FacturaDAO`/`PresupuestoDAO` migrados) y finalmente `UserDAO` (auth-sensible, `/security-review` extra, último de toda la cola). Patrón y orden de migración confirmados — ver Sprint B2-1→B2-15 arriba. Técnica de inicializador de campo + excepción comprobada (incl. constructores encadenados vía `this(...)` y try/catch autocontenido cuando el DAO hermano de la vista no está migrado) documentada y reutilizada en B2-2→B2-15. Los orquestadores siguen exigiendo Multi-IA real (Gemini+Codex) — no asumir mecánico sin consulta.
