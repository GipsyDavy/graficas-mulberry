...
**Última actualización:** 2026-06-22 (Sprint B2-16 — AlbaranDAO inyección Connection — 151/151 — Refactor B2 en 16/17 DAOs, cola de orquestadores CERRADA, solo queda UserDAO)
...
**HEAD:** commit Sprint B2-16 (más el commit de este STATE.md). Tests: 151/151. App funcional. Migración i18n de vistas: completa. Refactor B2 (Connection inyectada en DAOs): 16/17 DAOs migrados (`TarifaTramoDAO`, `NotaCalendarioDAO`, `ColumnConfigDAO`, `DynamicColumnValueDAO`, `ConsumoMaterialDAO`, `TarifaDAO`, `NominaDAO`, `PedidoDAO`, `MaterialDAO`, `PagoMaterialDAO`, `PagoPedidoDAO`, `EmpleadoDAO`, `ClienteDAO`, `PresupuestoDAO`, `FacturaDAO`, `AlbaranDAO`). Cola de riesgo moderado CERRADA. Cola de orquestadores CERRADA. **Queda solo `UserDAO`** (auth-sensible, último DAO de toda la cola, requiere `/security-review` extra además de VibeSec y Multi-IA obligatorio). Patrón y orden de migración confirmados — ver Sprint B2-1→B2-16 arriba. Técnica de inicializador de campo + excepción comprobada documentada y reutilizada en B2-2→B2-16. `UserDAO` cierra el Refactor B2 completo (17/17) — leer `LoginView`/`AdminSetupView`/`AuthService` antes de tocar nada, dado el carácter auth-sensible.
